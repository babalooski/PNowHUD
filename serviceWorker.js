...
chrome.runtime.onMessage.addListener(
	function(request, sender, sendResponse) {
		...
		if(request.command == "test"){
			...
			sendResponse({"confirmation": "test successful"});
		}
...
function analyze(stats, handLines){
	...
	try {
		...
		console.log("Hand object created successfully");
		var calc = new Calculator();
		console.log("Calculator created, processing hand...");
		var newStats = calc.processHand(hand, stats);
		console.log("Hand processing completed");
		return newStats;
	} catch (error) {
...
class Hand { //hand class
	...
	constructor(handLines){
		...
		console.log("Hand constructor called with:", handLines);
		
		// Simplified initialization for our format
		try {
			this.playerNumber = this.getPlayerNumber(this.handLines);
			this.players = this.getPlayers(this.handLines, this.playerNumber);
			...
			console.log("Hand constructor completed successfully");
		} catch (error) {
			console.error("Error in Hand constructor:", error);
...
class Calculator{
    ...
    extractPlayersFromHandLines(handLines){
        ...
        for(var i = 0; i < handLines.length; i++){
            ...
            // Skip the first line (player count) and last line (winner)
            if(i === 0 || line.includes('won')) continue;
            
            // Skip board lines but continue processing action lines after them
	...
	getHandsData(hand, HandsData, tableSize){
		var players = this.extractPlayersFromHandLines(hand.handLines);
		for(var i=0; i<players.length; i++){
			var username = players[i];
			var dataChange = [1,1]; //every hand we see of a player
			HandsData = this.updateData(HandsData, tableSize, username, dataChange);
		}
		return HandsData;
	...
	get3Bdata(hand, threeBetData, tableSize){ //3bet data
		// Simple 3B calculation based on hand lines
		var handLines = hand.handLines;
		var players = this.extractPlayersFromHandLines(handLines);
		var raiseCount = 0;
		var threeBettor = null;
		
		console.log("3B calculation - Hand lines:", handLines);
		console.log("3B calculation - Players found:", players);
		
    // Count raises and find the 3-bettor
    for(var i = 0; i < handLines.length; i++){
        var line = handLines[i];
        ...
        if(line.includes('board:')) continue;
			...
			if(parts.length >= 2){
				...
				if(action === 'raised'){
					raiseCount++;
					console.log("Raise detected, count:", raiseCount);
					
					// If this is the second raise, it's a 3-bet
					if(raiseCount === 2){
						threeBettor = player;
						console.log("3-bet detected by:", threeBettor);
					}
				}
			}
		}
		...
		for(var i = 0; i < players.length; i++){
			...
			threeBetData = this.updateData(threeBetData, tableSize, player, [0,1]); // Increment total hands
			
			if(player === threeBettor){
				threeBetData = this.updateData(threeBetData, tableSize, player, [1,0]); // Increment 3B hands
				console.log("Updated 3B for player:", player);
			}
		}
		
		console.log("3B data after update:", threeBetData);
		return threeBetData;
	}
	...
	getVPIPdata(hand, VPIPdata, tableSize){
        // Simple VPIP calculation based on hand lines
        var handLines = hand.handLines;
        var players = this.extractPlayersFromHandLines(handLines);
        var playersWhoPlayed = [];
        
        console.log("VPIP calculation - Hand lines:", handLines);
        console.log("VPIP calculation - Players found:", players);
        
    // Check each hand line for players who called or raised
    for(var i = 0; i < handLines.length; i++){
        var line = handLines[i];
        ...
        if(line.includes('board:')) continue;
            ...
            if(parts.length >= 2){
                ...
                // Check if player called or raised (VPIP actions)
                if(action === 'called' || action === 'raised' || action === 'bet'){
                    if(!playersWhoPlayed.includes(player)){
                        playersWhoPlayed.push(player);
                        console.log("Player added to VPIP list:", player);
                    }
                }
            }
        }
        
        console.log("Players who played (VPIP):", playersWhoPlayed);
        
        // Update stats for all players
        for(var i = 0; i < players.length; i++){
            var player = players[i];
            VPIPdata = this.updateData(VPIPdata, tableSize, player, [0,1]); // Increment total hands
            
            if(playersWhoPlayed.includes(player)){
                VPIPdata = this.updateData(VPIPdata, tableSize, player, [1,0]); // Increment VPIP hands
                console.log("Updated VPIP for player:", player);
            }
        }
        
        console.log("VPIP data after update:", VPIPdata);
        return VPIPdata;
	}
		
	getPFRdata(hand, PFRdata, tableSize){
        // Simple PFR calculation based on hand lines
        var handLines = hand.handLines;
        var players = this.extractPlayersFromHandLines(handLines);
        var playersWhoRaised = [];
        
        console.log("PFR calculation - Hand lines:", handLines);
        console.log("PFR calculation - Players found:", players);
        
        // Check each hand line for players who raised
        for(var i = 0; i < handLines.length; i++){
            var line = handLines[i];
            ...
            if(parts.length >= 2){
                ...
                // Check if player raised (PFR action)
                if(action === 'raised'){
                    if(!playersWhoRaised.includes(player)){
                        playersWhoRaised.push(player);
                        console.log("Player added to PFR list:", player);
                    }
                }
            }
        }
        
        console.log("Players who raised (PFR):", playersWhoRaised);
        
        // Update stats for all players
        for(var i = 0; i < players.length; i++){
            var player = players[i];
            PFRdata = this.updateData(PFRdata, tableSize, player, [0,1]); // Increment total hands
            
            if(playersWhoRaised.includes(player)){
                PFRdata = this.updateData(PFRdata, tableSize, player, [1,0]); // Increment PFR hands
                console.log("Updated PFR for player:", player);
            }
        }
        
        console.log("PFR data after update:", PFRdata);
        return PFRdata;
	}
    ...
    getAFdata(hand, AFdata, tableSize){
        // Simple AF calculation based on hand lines
        var handLines = hand.handLines;
        var players = this.extractPlayersFromHandLines(handLines);
        
        console.log("AF calculation - Hand lines:", handLines);
        console.log("AF calculation - Players found:", players);
        
        // Check each hand line for aggressive and passive actions
        for(var i = 0; i < handLines.length; i++){
            var line = handLines[i];
            ...
            if(parts.length >= 2){
                ...
                // Check if player is in the hand
                if(players.includes(player)){
                    if(action === 'called'){ // Passive action (denominator)
                        AFdata = this.updateData(AFdata, tableSize, player, [0,1]);
                        console.log("Added call for AF:", player);
                    }
                    if(action === 'bet' || action === 'raised'){ // Aggressive action (numerator)
                        AFdata = this.updateData(AFdata, tableSize, player, [1,0]);
                        console.log("Added aggressive action for AF:", player);
                    }
                }
            }
        }
        
        console.log("AF data after update:", AFdata);
        return AFdata;
	}
    ...
    handed(hand){
        ...
        if (parseInt(hand, 10) === hand){ //if an int is input
            ...
            // Extract player count from hand lines
            var players = this.extractPlayersFromHandLines(hand.handLines);
